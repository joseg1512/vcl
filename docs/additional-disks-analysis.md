...
**Estado:** **Approved / Implemented (phase 1)** — 2026-09-15.  
Jose approved this plan **without** optional phase 2 (no automatic VMFS inside nested ESXi). Phase 1 is implemented: table `imageadditionaldisk`, Manage Images → Advanced Options UI, libvirt/KVM empty qcow2 attach, VMware thin vmdk + SCSI attach, and capture ignores `*_adddiskN.vmdk`. Extra disks are ephemeral (recreated empty each load). Uncheck the box to delete all rows.

**How to use:** edit an image → Advanced Options → **Additional disks**. Checkbox off = current single OS disk. Checkbox on = up to 10 empty disks (size in GB) attached at reservation load on KVM and ESXi hosts.
