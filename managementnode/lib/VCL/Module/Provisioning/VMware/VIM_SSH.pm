...
=head2 wait_for_guest_tools
 ...
 Description : Polls "vim-cmd vmsvc/get.guest" until VMware Tools is running
               inside the guest. Used for nested ESXi bootstrap so guest
               operations can run before SSH is available.
...
sub wait_for_guest_tools {
	...
	notify($ERRORS{'OK'}, 0, "waiting up to $timeout_seconds seconds for VMware Tools on nested guest $computer_name (vmid $vm_id)");
	
	my $end_time = time() + $timeout_seconds;
	my $last_status = '';
	while (time() <= $end_time) {
		my ($exit_status, $output) = $self->_run_vim_cmd("vmsvc/get.guest $vm_id", 60, 2);
		if ($output && @$output) {
			my $joined = join("\n", @$output);
			my ($tools_status) = $joined =~ /toolsStatus\s*=\s*"([^"]+)"/;
			my ($tools_running) = $joined =~ /toolsRunningStatus\s*=\s*"([^"]+)"/;
			my ($ip_address) = $joined =~ /ipAddress\s*=\s*"([\d\.]+)"/;
			...
			notify($ERRORS{'DEBUG'}, 0, "vmsvc/get.guest not available yet for vmid $vm_id");
		}
		sleep 5;
	}
...
=head2 guest_run_script
 ...
 Returns     : hashref {ok, output} or undef
 Description : Runs a shell script inside the guest via GuestOperationsManager
               (Python helper talking to the metal host SDK). Does not use SSH
               into the guest.
...
sub guest_run_script {
	...
	my $script_file = $self->_guestops_temp_path('script.sh');
	if (!_guestops_write_file($script_file, $script_text)) {
		return;
	}
	
	my $result = $self->_guestops_python(
		'run',
		$guest_username,
		$guest_password,
		$timeout_seconds,
	...
	unlink $script_file if -e $script_file;
	return $result;
}
...
=head2 guest_write_file
 ...
 Returns     : boolean
 Description : Writes a file inside the guest via GuestFileManager (no SSH).
...
sub guest_write_file {
	...
	my $local_file = $self->_guestops_temp_path('upload.dat');
	if (!_guestops_write_file($local_file, $contents)) {
		return;
	}
	...
	my $result = $self->_guestops_python(
		...
		$guest_password,
		120,
		['--guest-path', $guest_path, '--file', $local_file],
	);
	unlink $local_file if -e $local_file;
	if ($result && $result->{ok}) {
		notify($ERRORS{'DEBUG'}, 0, "wrote guest file via guest operations: $guest_path");
		return 1;
	}
	notify($ERRORS{'WARNING'}, 0, "failed to write guest file $guest_path: " . ($result ? $result->{output} : 'undef'));
	return;
}

#//////////////////////////////////////////////////////////////////////////////

=head2 _guestops_python

 Parameters  : $action, $guest_username, $guest_password, $timeout_seconds, $extra_args_arrayref
 Returns     : hashref {ok, output}
 Description : Invokes guest_ops.py against the metal host SDK.

=cut

sub _guestops_python {
	my $self = shift;
	my ($action, $guest_username, $guest_password, $timeout_seconds, $extra_args) = @_;
	
	my $helper_path = $self->_guestops_helper_path();
	if (!$helper_path) {
		notify($ERRORS{'WARNING'}, 0, "guest_ops.py helper was not found next to VIM_SSH.pm");
		return;
	}
	
	my $python = $self->_guestops_python_bin();
	if (!$python) {
		notify($ERRORS{'WARNING'}, 0, "python3 was not found on the management node; cannot run guest operations helper");
		return;
	}
	
	my $sdk = $self->_guestops_sdk_connection();
	if (!$sdk) {
		return;
	}
	...
	my $vmx_datastore_path = '';
	if ($vmx_file_path && $self->can('_get_datastore_path')) {
		$vmx_datastore_path = $self->_get_datastore_path($vmx_file_path) || '';
	}
	...
	my @cmd = (
		...
		'--guest-user', $guest_username || 'root',
	);
	push @cmd, '--vm-id', $vm_id if $vm_id;
	push @cmd, '--vmx-path', $vmx_datastore_path if $vmx_datastore_path;
	push @cmd, @$extra_args if $extra_args && ref($extra_args) eq 'ARRAY';
	
	# Quote for the shell; passwords stay in the environment
	my $quoted = join(' ', map { $self->_guestops_shell_quote($_) } @cmd);
	
	notify($ERRORS{'DEBUG'}, 0, "running guest operations helper: $python $helper_path $action (vm id $vm_id)");
	
	local $ENV{VCL_VMHOST_PASSWORD} = $sdk->{password};
	local $ENV{VCL_GUEST_PASSWORD} = defined($guest_password) ? $guest_password : '';
	
	my $run_timeout = ($timeout_seconds || 180) + 60;
	my ($exit_status, $output) = run_command($quoted, 1, $run_timeout);
	
	my $joined = defined($output) ? join("\n", @$output) : '';
	if (!defined($output)) {
		notify($ERRORS{'WARNING'}, 0, "failed to execute guest operations helper");
		return {ok => 0, output => ''};
	}
	
	if ($joined =~ /^OK\b/m || (defined($exit_status) && $exit_status eq '0' && $action eq 'wait-tools' && $joined =~ /guestToolsRunning/)) {
		my $body = $joined;
		$body =~ s/^OK\s*//;
		return {ok => 1, output => $body};
	}
	
	notify($ERRORS{'WARNING'}, 0, "guest operations helper '$action' failed, exit $exit_status, output:\n$joined");
	return {ok => 0, output => $joined};
}

#//////////////////////////////////////////////////////////////////////////////

sub _guestops_helper_path {
	my $self = shift;
	my $path = __FILE__;
	$path =~ s/VIM_SSH\.pm$/guest_ops.py/;
	return $path if -r $path;
	notify($ERRORS{'DEBUG'}, 0, "guest_ops.py not readable at $path");
	return;
}

sub _guestops_python_bin {
	for my $bin ('python3', '/usr/bin/python3', '/usr/local/bin/python3', 'python') {
		my ($exit_status, $output) = run_command("$bin -c 'import sys; print(sys.version_info[0])'", 1, 15);
		if (defined($output) && grep(/^3/, @$output)) {
			return $bin;
		}
	}
	return;
...
sub _guestops_temp_path {
	my $self = shift;
	my $suffix = shift || 'dat';
	my $dir = '/tmp';
	return sprintf('%s/vcl-guestops-%s-%s-%s', $dir, $$, time(), $suffix);
}

sub _guestops_write_file {
	my ($path, $contents) = @_;
	if (!open(my $fh, '>', $path)) {
		notify($ERRORS{'WARNING'}, 0, "failed to write temp file $path: $!");
		return;
	}
	else {
		print $fh $contents;
...
sub _guestops_sdk_connection {
	my $self = shift;
	
	my $vmhost_hostname = $self->data->get_vmhost_hostname();
	my $vmhost_username = $self->data->get_vmhost_profile_username(0);
	my $vmhost_password = $self->data->get_vmhost_profile_password(0);
	
	if (!$vmhost_username || !$vmhost_password) {
		notify($ERRORS{'WARNING'}, 0, "guest operations require vmprofile username/password to reach the metal host SDK (host $vmhost_hostname)");
		return;
	}
	
	my $target = $vmhost_hostname;
	eval { $target = determine_remote_connection_target($vmhost_hostname) || $vmhost_hostname; };
