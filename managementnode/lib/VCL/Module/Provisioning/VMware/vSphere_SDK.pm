...
=head2 wait_for_guest_tools

 Parameters  : $timeout_seconds (optional, default 480)
 Returns     : hashref {tools_running, tools_status, ip_address} or false
 Description : Polls the vSphere SDK VirtualMachine guest object until VMware
               Tools is running. Used for nested ESXi bootstrap before SSH.

=cut

sub wait_for_guest_tools {
	my $self = shift;
	if (ref($self) !~ /VCL::Module/i) {
		notify($ERRORS{'CRITICAL'}, 0, "subroutine was called as a function, it must be called as a class method");
		return;
	...
	my $timeout_seconds = shift || 480;
	my $computer_name = $self->data->get_computer_short_name() || '';
	
	notify($ERRORS{'OK'}, 0, "waiting up to $timeout_seconds seconds for VMware Tools on $computer_name via vSphere SDK");
	
	local $SIG{__DIE__} = sub{};
	my $end_time = time() + $timeout_seconds;
	my $last_status = '';
	while (time() <= $end_time) {
		my $vm_view = $self->_get_vm_view();
		if ($vm_view) {
			eval { $vm_view->update_view_data(); };
			my $guest = eval { $vm_view->guest };
			if ($guest) {
				my $tools_running = eval { $guest->toolsRunningStatus } || '';
				my $tools_status = eval { $guest->toolsStatus } || '';
				my $ip_address = eval { $guest->ipAddress } || '';
				$last_status = "toolsStatus=$tools_status toolsRunningStatus=$tools_running ipAddress=$ip_address";
				if ($tools_running =~ /guestToolsRunning/i && $tools_status !~ /toolsNotRunning/i) {
					notify($ERRORS{'OK'}, 0, "VMware Tools running on $computer_name: $last_status");
					return {
				...
				notify($ERRORS{'DEBUG'}, 0, "VMware Tools not ready on $computer_name: $last_status");
			}
		}
		sleep 5;
	}
	
	notify($ERRORS{'WARNING'}, 0, "timed out waiting for VMware Tools on $computer_name: $last_status");
	return;
}

#//////////////////////////////////////////////////////////////////////////////

=head2 guest_run_script

 Parameters  : $script_text, $guest_username, $guest_password, $timeout_seconds (optional)
 Returns     : hashref {ok, output}
 Description : Uploads a shell script and runs it in the guest via
               GuestOperationsManager. Does not use SSH into the guest.

=cut

sub guest_run_script {
	my $self = shift;
	if (ref($self) !~ /VCL::Module/i) {
		notify($ERRORS{'CRITICAL'}, 0, "subroutine was called as a function, it must be called as a class method");
		return;
	...
	my ($script_text, $guest_username, $guest_password, $timeout_seconds) = @_;
	if (!defined($script_text) || !length($script_text)) {
		notify($ERRORS{'WARNING'}, 0, "script text argument was not supplied");
		return;
	}
	$guest_username = $guest_username || 'root';
	$timeout_seconds = $timeout_seconds || 180;
	
	my $remote_script = '/tmp/vcl-mn-guestops.sh';
	my $remote_wrap = '/tmp/vcl-mn-guestops-wrap.sh';
	my $remote_out = '/tmp/vcl-mn-guestops.out';
	
	if (!$self->guest_write_file($remote_script, $script_text, $guest_username, $guest_password)) {
		notify($ERRORS{'WARNING'}, 0, "failed to upload guest operations script");
		return {ok => 0, output => ''};
	}
	
	my $wrapper = "#!/bin/sh\n/bin/sh $remote_script > $remote_out 2>&1\necho __VCL_GUESTOPS_RC__\$? >> $remote_out\n";
	if (!$self->guest_write_file($remote_wrap, $wrapper, $guest_username, $guest_password)) {
		notify($ERRORS{'WARNING'}, 0, "failed to upload guest operations wrapper");
		return {ok => 0, output => ''};
	}
	...
	my $pid;
	eval {
		my $vm_view = $self->_get_vm_view() || die "no VM view\n";
		my ($auth, $process_mgr) = $self->_guestops_managers($guest_username, $guest_password);
		die "no guest process manager\n" if !$process_mgr;
		my $spec = GuestProgramSpec->new(
			programPath => '/bin/sh',
			arguments => $remote_wrap,
			workingDirectory => '/',
		);
		$pid = $process_mgr->StartProgramInGuest(vm => $vm_view, auth => $auth, spec => $spec);
	};
	if ($EVAL_ERROR || !$pid) {
		notify($ERRORS{'WARNING'}, 0, "StartProgramInGuest failed: $EVAL_ERROR");
		return {ok => 0, output => $EVAL_ERROR || ''};
	}
	
	notify($ERRORS{'DEBUG'}, 0, "started guest program pid $pid, waiting up to $timeout_seconds seconds");
	my $end_time = time() + $timeout_seconds;
	my $finished = 0;
	while (time() <= $end_time) {
		my $ended = 0;
		eval {
			my $vm_view = $self->_get_vm_view() || die "no VM view\n";
			my ($auth, $process_mgr) = $self->_guestops_managers($guest_username, $guest_password);
			my $procs = $process_mgr->ListProcessesInGuest(vm => $vm_view, auth => $auth, pids => [$pid]);
			if ($procs && @$procs && $procs->[0]->endTime) {
				$ended = 1;
			}
		};
		if ($ended) {
	...
	if (!$finished) {
		notify($ERRORS{'WARNING'}, 0, "guest program pid $pid did not exit within $timeout_seconds seconds");
		return {ok => 0, output => ''};
	}
	
	my $output = $self->_guestops_read_file($remote_out, $guest_username, $guest_password);
	$output = '' if !defined($output);
	notify($ERRORS{'DEBUG'}, 0, "guest program pid $pid finished, output length " . length($output));
	return {ok => 1, output => $output};
}

#//////////////////////////////////////////////////////////////////////////////

=head2 guest_write_file

 Parameters  : $guest_path, $contents, $guest_username, $guest_password
 Returns     : boolean
 Description : Writes a file inside the guest via GuestFileManager.

=cut

sub guest_write_file {
	my $self = shift;
	if (ref($self) !~ /VCL::Module/i) {
		notify($ERRORS{'CRITICAL'}, 0, "subroutine was called as a function, it must be called as a class method");
		return;
	...
	my ($guest_path, $contents, $guest_username, $guest_password) = @_;
	if (!$guest_path) {
		notify($ERRORS{'WARNING'}, 0, "guest path argument was not supplied");
		return;
	}
	...
	eval {
		my $vm_view = $self->_get_vm_view() || die "no VM view\n";
		my ($auth, $process_mgr, $file_mgr) = $self->_guestops_managers($guest_username, $guest_password);
		die "no guest file manager\n" if !$file_mgr;
		my $attr = GuestFileAttributes->new();
		$url = $file_mgr->InitiateFileTransferToGuest(
			vm => $vm_view,
			auth => $auth,
			guestFilePath => $guest_path,
			fileAttributes => $attr,
			fileSize => length($contents),
			overwrite => 1,
		);
	};
	if ($EVAL_ERROR || !$url) {
		notify($ERRORS{'WARNING'}, 0, "InitiateFileTransferToGuest failed for $guest_path: $EVAL_ERROR");
		return;
	}
	
	if (!$self->_guestops_http_put($url, $contents)) {
		notify($ERRORS{'WARNING'}, 0, "failed to PUT guest file contents to $guest_path");
		return;
	}
	notify($ERRORS{'DEBUG'}, 0, "wrote guest file via vSphere SDK guest operations: $guest_path");
	return 1;
}

#//////////////////////////////////////////////////////////////////////////////

sub _guestops_managers {
	my $self = shift;
	my ($guest_username, $guest_password) = @_;
	...
	my $service_content = $self->_get_service_content() || return;
	my $gom_ref = eval { $service_content->{guestOperationsManager} };
	if (!$gom_ref) {
		$gom_ref = eval { $service_content->guestOperationsManager };
	}
	if (!$gom_ref) {
		notify($ERRORS{'WARNING'}, 0, "guestOperationsManager is not available on this VM host");
		return;
	}
	my $gom = $self->_get_view($gom_ref) || return;
	my $process_mgr = $self->_get_view($gom->processManager);
	my $file_mgr = $self->_get_view($gom->fileManager);
	return ($auth, $process_mgr, $file_mgr);
}

sub _guestops_read_file {
	my $self = shift;
	my ($guest_path, $guest_username, $guest_password) = @_;
	...
	if ($EVAL_ERROR || !$info) {
		notify($ERRORS{'WARNING'}, 0, "InitiateFileTransferFromGuest failed for $guest_path: $EVAL_ERROR");
		return;
	}
	
	my $url = eval { $info->url } || $info;
	return $self->_guestops_http_get($url);
}

sub _guestops_sdk_host {
	my $self = shift;
	my $url = '';
	eval { $url = Opts::get_option('url'); };
	if ($url && $url =~ m{https?://([^/:]+)}) {
		return $1;
	}
	my $vmhost_hostname = $self->data->get_vmhost_hostname();
	eval { $vmhost_hostname = determine_remote_connection_target($vmhost_hostname) || $vmhost_hostname; };
	return $vmhost_hostname;
}

sub _guestops_fix_transfer_url {
	my $self = shift;
	my $url = shift || return;
	if ($url =~ m{https://\*}) {
		my $host = $self->_guestops_sdk_host();
		$url =~ s{https://\*}{https://$host};
	}
	return $url;
}

sub _guestops_http_put {
	my $self = shift;
	my ($url, $contents) = @_;
	$url = $self->_guestops_fix_transfer_url($url);
	
	my $temp_path = sprintf('/tmp/vcl-guestops-put-%s-%s', $$, time());
	if (!open(my $fh, '>', $temp_path)) {
		notify($ERRORS{'WARNING'}, 0, "failed to write temp PUT file $temp_path: $!");
		return;
	}
	else {
		binmode $fh;
	...
	my $command = "curl -sk --max-time 60 -X PUT --data-binary \@$temp_path " . $self->_guestops_shell_quote($url);
	my ($exit_status, $output) = run_command($command, 1, 90);
	unlink $temp_path;
	if (defined($exit_status) && $exit_status eq '0') {
		return 1;
	}
	notify($ERRORS{'WARNING'}, 0, "curl PUT to guest file URL failed, exit $exit_status, output:\n" . (defined($output) ? join("\n", @$output) : ''));
	return;
}

sub _guestops_http_get {
	my $self = shift;
	my ($url) = @_;
	$url = $self->_guestops_fix_transfer_url($url);
	my $command = "curl -sk --max-time 60 " . $self->_guestops_shell_quote($url);
	my ($exit_status, $output) = run_command($command, 1, 90);
	if (defined($exit_status) && $exit_status eq '0') {
		return defined($output) ? join("\n", @$output) : '';
	}
	notify($ERRORS{'WARNING'}, 0, "curl GET of guest file URL failed, exit $exit_status");
	return;
}

sub _guestops_shell_quote {
	my $self = shift;
